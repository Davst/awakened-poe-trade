<template>
  <div v-if="show" class="p-4 layout-column min-h-0">
    ...
    <div v-if="item.disenchantCandidates" class="grid" style="grid-template-columns: auto auto;">
      <div v-for="uniqueItemDisenchanting in item.disenchantCandidates" class="flex" style="padding-left: 9px">
        <div v-if="(item.disenchantCandidates.length != 1)">
          <div style="padding-right: 2px;">{{ uniqueItemDisenchanting.name }}</div>
        </div>
        <div>
